...
cout<<endl;
		...
		cout<<"                                  *************************************"<<endl;
		cout<<endl;
	cout<<"					{1} Register"<<endl<<"					{2} Login"<<endl<<"					{3} Login As Guest"<<endl;
	cout<<endl;
	cout<<"                                  *************************************"<<endl;
	cout<<endl;
	cin>>choicex;
	cout<<"***********************************PLEASE WAIT WHILE INFORMATION IS BEING LOADED***********************************\n";
	Sleep(1500);
	if(choicex==1)
	{
...
system("CLS");
cout<<"WELCOME TO ZUQU DRY CLEANING SHOP\n\n\n";
	int s = 1, z = 0;
	a[0].others = "{1} WASH \n{2} DRY\n{3} IRON\n";
	while (s != 0)
	{
		...
		if (a[0].choice == 1)
		{
			a[1].others = "\t\t\t\t**WHAT WOULD YOU LIKE TO DO**\n {1} JEANS\n {2} SWEATERS \n {3} SHIRTS \n {4}TSHIRTS\n {5}PAJAMAS \n {6} BLANKETS \n {7} BEDSHEETS\n {8} CURTAINS\n {9} COAT\n {10} WHOLE SUIT\n {11} SOCKS\n";
			cout << a[1].others;
			cin >> a[1].choice;
			if (a[1].choice == 1)
			{
		...
		a[1].total = a[1].total + a[0].total;
			cout << "your current total is " << a[1].total << "\n";
...
void clean(int& e, string& b, char& d)
{
	...
	if (d == 'y')
	{
		...
		a[3].others = "do you want to perform dry or iron on washed clothes?\nPress 1 to dry only\nPress 2 to iron only \nPress 3 for both\nPress 4 if you dont want to do either\n";
		cout << a[3].others;
		cin >> a[3].choice;
		if (a[3].choice == 1)
		{
